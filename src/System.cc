...
namespace ORB_SLAM3 {
    ...
    cv::Mat System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp,
                                const vector<IMU::Point> &vImuMeas, string filename, const string &resultPath) {
        ...
        cv::FileStorage mpfile(
                resultPath + "Results_Mappoints/" + std::to_string(mpTracker->mCurrentFrame.mnId) + ".xml",
        ...
        for (auto &mTrackedMapPoint : mTrackedMapPoints) {
            ...
            if (pMp) {
                ...
                if (PcZ > 0.0f) {
                    ...
                    if (u >= -0.5 && v >= -0.5 && u <= (imLeft.cols + 0.5) && v <= (imLeft.rows + 0.5)) {
        ...
        cv::Mat Twc = mpTracker->mCurrentFrame.mTcw.inv();
        cv::Mat Rwc = Twc.colRange(0, 3).rowRange(0, 3);
        cv::Mat twc = Twc.rowRange(0, 3).col(3);

        ofstream f;
        filename = "LocalCameraTrajectory.txt";
    ...
    void System::SaveTrajectoryKITTI(const string &filename, const string &anchorPath) {
        cout << endl << "Saving camera trajectory to " << filename << " ..." << endl;
        if (mSensor == MONOCULAR) {
        ...
        for (list<cv::Mat>::iterator lit = mpTracker->mlRelativeFramePoses.begin(), lend = mpTracker->mlRelativeFramePoses.end();
            ...
            // save the mappoint as anchor point when all BA are done
//        cv::Mat AnchorMap = cv::Mat::zeros(mpTracker->mImGray.rows, mpTracker->mImGray.cols, CV_32FC1);
//        cv::FileStorage mpfile(anchorPath + "Results_Mappoints_Global/" + std::to_string(mpTracker->mCurrentFrame.mnId) + ".xml", cv::FileStorage::WRITE);
//        mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;
//        for (auto & mTrackedMapPoint : mTrackedMapPoints) {
//            MapPoint* pMp = mTrackedMapPoint;
//            if (pMp){
...
//                    float u=ORB_SLAM3::Frame::fx*PcX*invz+ORB_SLAM3::Frame::cx;
//                    float v=ORB_SLAM3::Frame::fy*PcY*invz+ORB_SLAM3::Frame::cy;
//                    if(u>=-0.5 && v>=-0.5 && u<=(mpTracker->mImGray.cols+0.5) && v<=(mpTracker->mImGray.rows+0.5)){
//                        AnchorMap.at<float>(static_cast<int>(v), static_cast<int>(u)) = PcZ;
//                    }
//                }
//            }
